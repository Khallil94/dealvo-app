...
const NotificationService = {
    ...
    try {
      await fetch('https://exp.host/--/api/v2/push/send', {
        ...
        body: JSON.stringify({
          ...
          sound: 'default',
          badge: 1,
          channelId: data.type === 'price_alert' ? 'price_alerts' : 'deals',
          priority: 'high',
        }),
      });
    } catch (e) {
  ...
  //
  async setPriceAlert(userId, productId, productTitle, currentPrice) {
...
const HomeTab = React.memo((props) => {
  ...
  var {setSearch, sortMode, setSortMode, cart, favorites, setDetail, toggleFav, addToCart, openShare, setShowCountry} = props;
  ...
  return (
    <View style={{ flex:1, backgroundColor:T.bg }}>
      ...
      <View style={s.homeHeader}>
        <View>
          <Text style={{ color:T.textSub, fontSize:13 }}>{greeting},</Text>
          <Text style={{ color:T.text, fontSize:22, fontWeight:'900' }}>{user?.name||'Guest'} 👋</Text>
        </View>
      ...
      <View style={s.searchBar}>
        <Text style={{ fontSize:16, color:T.textMuted, marginRight:8 }}>🔍</Text>
        <TextInput style={s.searchInput}
          placeholder={`Search on ${plat.name}...`} placeholderTextColor={T.textMuted}
...
const WishlistTab = React.memo((props) => {
  var {products, favorites, cart, country, setDetail, toggleFav, addToCart} = props;
  const items = useMemo(() => products.filter(p=>favorites.includes(p.id)), [products, favorites]);
  return (
    <View style={{ flex:1, backgroundColor:T.bg }}>
      <View style={s.tabHdr}>
        <Text style={s.tabHdrTitle}>❤️ Wishlist</Text>
        <Text style={{ color:T.textMuted, fontSize:13 }}>{items.length} items</Text>
      </View>
      {items.length===0 ? (
        <View style={s.empty}>
          <Text style={{ fontSize:52 }}>💔</Text>
          <Text style={s.emptyTitle}>Your wishlist is empty</Text>
          <Text style={{ color:T.textMuted, fontSize:13, marginTop:4 }}>Tap ❤️ on any product to save it</Text>
        </View>
      ) : (
...
const CartTab = React.memo(function CartTab(props) {
  var { products, cart, setCart, country } = props;
  ...
  return (
    <View style={{ flex:1, backgroundColor:T.bg }}>
      <View style={s.tabHdr}>
        <Text style={s.tabHdrTitle}>🛒 Cart</Text>
        <Text style={{ color:T.textMuted, fontSize:13 }}>{cart.length} items</Text>
      </View>
      {items.length===0 ? (
        <View style={s.empty}>
          <Text style={{ fontSize:52 }}>🛍️</Text>
          <Text style={s.emptyTitle}>Your cart is empty</Text>
          <Text style={{ color:T.textMuted, fontSize:13, marginTop:4 }}>Tap + on any product to add it</Text>
        </View>
      ) : (
          ...
          renderItem={({item}) => {
            ...
            return (
              <View style={s.cartRow}>
                ...
                <View style={{ flex:1 }}>
                  <Text style={{ color:T.text, fontWeight:'700', fontSize:13 }} numberOfLines={2}>{item.title}</Text>
          ...
          ListFooterComponent={
            <View style={s.cartFooter}>
              <View style={{ flexDirection:'row', justifyContent:'space-between', marginBottom:14 }}>
                <Text style={{ color:T.textSub, fontSize:14 }}>Subtotal ({items.length} items)</Text>
              ...
              <Text style={{ color:T.textMuted, fontSize:11, textAlign:'center', marginBottom:12 }}>
                * Prices in {country.currency}. Final price may vary by platform.
              </Text>
              <TouchableOpacity style={[s.primBtn, { backgroundColor:T.primary }]}>
                <Text style={s.primBtnTxt}>Proceed to Checkout ></Text>
              </TouchableOpacity>
            </View>
          }
        />
...
const ProfileTab = React.memo((props) => {
  var {user, favorites, cart, onSignOut, country, onContact, darkMode, onToggleDark} = props;
  //
  return (
    <ScrollView style={{ flex:1, backgroundColor:T.bg }} contentContainerStyle={{ padding:20 }}>
      <Text style={[s.tabHdrTitle, { marginBottom:20 }]}>👤 Profile</Text>
        ...
        <View style={{ flexDirection:'row', gap:8, marginTop:10, flexWrap:'wrap', justifyContent:'center' }}>
          {user?.isGuest && (
            <View style={[s.badge, { backgroundColor:T.yellow+'30' }]}>
              <Text style={[s.badgeTxt, { color:T.text }]}>Guest Mode</Text>
            </View>
          )}
        </View>
      </View>

      {/* Wishlist & Cart Count فقط — بدون كليكات */}
      <View style={s.statsRow}>
        {[['❤️','Saved', favorites.length], ['🛒','Cart', cart.length]].map(([icon,label,val]) => (
      ...
      {/* Country */}
      <View style={[s.profileCard, { flexDirection:'row', alignItems:'center', padding:16, marginTop:4 }]}>
        <Text style={{ fontSize:26, marginRight:12 }}>{country.flag}</Text>
        <View style={{ flex:1 }}>
          <Text style={{ fontWeight:'700', color:T.text }}>{country.name}</Text>
          <Text style={{ color:T.textMuted, fontSize:12 }}>Shopping region - {country.currency}</Text>
        </View>
      </View>

      {/* Dark Mode Toggle */}
      <View style={{ flexDirection:'row', alignItems:'center', backgroundColor:T.inputBg, borderRadius:16, padding:16, marginTop:8 }}>
        <Text style={{ fontSize:22, marginRight:12 }}>{darkMode ? '🌙' : '☀️'}</Text>
        <View style={{ flex:1 }}>
          <Text style={{ fontWeight:'700', color:T.text }}>Dark Mode</Text>
          <Text style={{ color:T.textMuted, fontSize:12 }}>{darkMode ? 'On' : 'Off'}</Text>
        </View>
        <TouchableOpacity
      ...
      <TouchableOpacity
        ...
        onPress={onContact}
      >
        <Text style={s.primBtnTxt}>📬 Contact Us</Text>
      </TouchableOpacity>
      ...
      <TouchableOpacity
        ...
        onPress={onSignOut}
      >
        <Text style={s.primBtnTxt}>🚪 Sign Out</Text>
      </TouchableOpacity>
...
export default function App() {
  ...
  const handlePriceAlert = useCallback(async (product) => {
    ...
    if (success) {
      showToast('🔔 Alert set! We will notify you when price drops.');
    } else {
  ...
  return (
    <SafeAreaView style={{ flex:1, backgroundColor:T.bg }}>
      ...
      {tab === 'social' && (
        <SocialTab user={user} country={country} showToast={showToast} />
      )}
      {tab === 'rewards' && (
        <RewardsTab user={user} showToast={showToast} />
      )}
      {tab === 'profile' && (
        <ProfileTab
      ...
      <View style={s.bottomNav}>
        {[
          ...
          { id:'wishlist',  icon:'❤️', label:'Saved',  badge:favorites.length },
          { id:'cart',      icon:'🛒', label:'Cart',   badge:cart.length },
          ...(user?.isAdmin ? [{ id:'dashboard', icon:'📊', label:'Admin' }] : []),
