...
const COUNTRIES = [
  ...
  { id:'kw',  flag:'🇰🇼', name:'Kuwait',           domain:'aliexpress.com',  currency:'KWD', tag:'dv-kw21' },
  { id:'qa',  flag:'🇶🇦', name:'Qatar',            domain:'aliexpress.com',  currency:'QAR', tag:'dv-qa21' },
  { id:'bh',  flag:'🇧🇭', name:'Bahrain',          domain:'aliexpress.com',  currency:'BHD', tag:'dv-bh21' },
  { id:'om',  flag:'🇴🇲', name:'Oman',             domain:'aliexpress.com',  currency:'OMR', tag:'dv-om21' },
...
// Category chip مع fadeUp animation
const AnimatedCatChip = React.memo(function AnimatedCatChip({ c, idx, category, setCategory, setSearch }) {
  const anim = useRef(new Animated.Value(0)).current;
  useEffect(() => {
  ...
  return (
    <Animated.View style={{
      ...
      <TouchableOpacity
        style={category===c.id ? [s.catChip, { backgroundColor:c.color }] : s.catChip}
        onPress={() => { setCategory(c.id); setSearch(''); }}
      >
        <Text style={{ fontSize:14 }}>{c.icon}</Text>
        <Text style={category===c.id ? [s.catChipTxt, { color:'#fff' }] : s.catChipTxt}>{c.label}</Text>
      </TouchableOpacity>
    </Animated.View>
  );
});

// Trending card مع slideIn من اليمين
const AnimatedTrendCard = React.memo(function AnimatedTrendCard({ p, idx, country, setDetail }) {
  const slideAnim = useRef(new Animated.Value(80)).current;
  const fadeAnim  = useRef(new Animated.Value(0)).current;
  useEffect(() => {
    Animated.parallel([
      Animated.timing(slideAnim, { toValue:0, duration:400, delay: idx * 80, useNativeDriver:true }),
      Animated.timing(fadeAnim,  { toValue:1, duration:400, delay: idx * 80, useNativeDriver:true }),
    ]).start();
  }, []);
  return (
...
// Product card مع fadeUp متدرج
const AnimatedProductCard = React.memo(function AnimatedProductCard({ item, idx, cart, favorites, country, onPress, onFav, onCart }) {
  const anim  = useRef(new Animated.Value(0)).current;
  const slide = useRef(new Animated.Value(30)).current;
  useEffect(() => {
    Animated.parallel([
      Animated.timing(anim,  { toValue:1, duration:400, delay: idx * 50, useNativeDriver:true }),
      Animated.timing(slide, { toValue:0, duration:400, delay: idx * 50, useNativeDriver:true }),
    ]).start();
  }, []);
  return (
...
// Hero Banner مع slideIn من اليسار
const AnimatedHeroBanner = React.memo(function AnimatedHeroBanner({ plat }) {
...
function SplashScreen({ onDone }) {
  ...
  useEffect(() => {
    Animated.sequence([
      ...
      Animated.timing(barW, { toValue:1, duration:900, useNativeDriver:false }),
    ]).start();
    ...
    <View style={{ flex:1, backgroundColor:'#0A0A12', alignItems:'center', justifyContent:'center' }}>
      ...
      <Animated.View style={{ opacity:fade, transform:[{scale}], alignItems:'center' }}>
        <View style={{
          ...
          marginBottom:24, shadowColor:'#FF6B35', shadowOpacity:0.5, shadowRadius:30, elevation:20,
        }}>
          <Text style={{ fontSize:52 }}>🛍️</Text>
        </View>
        <Text style={{ fontSize:42, fontWeight:'900', color:'#FFF', letterSpacing:8 }}>DEALVO</Text>
        <Animated.Text style={{
          fontSize:12, color:'rgba(255,255,255,0.4)', marginTop:8, letterSpacing:3,
          opacity: tagline,
        ...
        <Animated.Text style={{ color:'rgba(255,255,255,0.2)', fontSize:10, letterSpacing:3, marginTop:10, opacity:tagline }}>
          LOADING
        </Animated.Text>
      </Animated.View>
      <Animated.Text style={{
        position:'absolute', bottom:50, color:'rgba(255,255,255,0.12)',
        fontSize:10, letterSpacing:2, opacity:tagline,
      }}>
        Amazon & AliExpress
      </Animated.Text>
    </View>
...
function OnboardingScreen({ onComplete }) {
  ...
  const goNext = () => {
    if (page < ONBOARD_PAGES.length - 1) { goTo(page + 1); }
    else { onComplete(); }
  };

  // Onboarding pages data داكنة
  const DARK_PAGES = [
    {
      icon:'🛍️', color:'#FF6B35',
      title:'Welcome to DEALVO',
      desc:'Find the best deals on Amazon and AliExpress — all in one place.',
      bg:'#1A0800',
    },
    {
      icon:'🔔', color:'#FF9500',
      title:'Price Alerts',
      desc:'Never miss a price drop. Get instant notifications when prices fall.',
      bg:'#1A0F00',
      preview: [
        { platform:'📦 Amazon Alert!', item:'Sony WH-1000XM5 price dropped!', old:'$349', price:'$249', save:'Save $100', color:'#FF9900' },
        { platform:'⚡ AliExpress Flash!', item:'Wireless Earbuds — 2 hours only!', old:'$45', price:'$12', save:'Save $33', color:'#E62C00' },
      ],
    },
    {
      icon:'💰', color:'#00C896',
      title:'Earn Commissions',
      desc:'Share deals with friends. Every purchase through your link earns you money.',
      bg:'#001A12',
    },
    {
      icon:'🚀', color:'#7C3AED',
      title:'Start Saving Today',
      desc:'Join thousands of smart shoppers finding the best deals every day.',
      bg:'#0A0018',
      stats: [
        { value:'10K+', label:'Products' },
        { value:'$500', label:'Avg. Saved' },
        { value:'50K+', label:'Users' },
      ],
    },
  ];

  const dp = DARK_PAGES[page];

  return (
    <View style={{ flex:1, backgroundColor: dp.bg }}>
      <StatusBar style="light" />

      {/* Skip */}
      <TouchableOpacity
        style={{ position:'absolute', top: Platform.OS==='ios'?60:36, right:20, zIndex:10,
          backgroundColor:'rgba(255,255,255,0.1)', paddingHorizontal:16, paddingVertical:8, borderRadius:20 }}
        onPress={onComplete}
      >
        <Text style={{ color:'rgba(255,255,255,0.6)', fontWeight:'600', fontSize:13 }}>Skip</Text>
      </TouchableOpacity>

      {/* Page counter */}
      <View style={{ position:'absolute', top: Platform.OS==='ios'?62:38, left:20, zIndex:10 }}>
        <Text style={{ color:'rgba(255,255,255,0.3)', fontSize:13, fontWeight:'700' }}>
          {String(page+1).padStart(2,'0')} / {String(DARK_PAGES.length).padStart(2,'0')}
        </Text>
      </View>

      {/* ScrollView قابل للسحب */}
      ...
      <Animated.View style={{
        ...
        <View style={{
          ...
          borderWidth:1, borderColor: dp.color + '40',
          alignItems:'center', justifyContent:'center', marginBottom:32,
          shadowColor: dp.color, shadowOpacity:0.3, shadowRadius:20, elevation:10,
        }}>
          <Text style={{ fontSize:60 }}>{dp.icon}</Text>
        </View>

        {/* Label */}
        <View style={{ backgroundColor: dp.color + '20', paddingHorizontal:14, paddingVertical:5, borderRadius:20, marginBottom:16 }}>
          <Text style={{ color: dp.color, fontSize:11, fontWeight:'800', letterSpacing:2 }}>
            {page === 0 ? 'WELCOME' : page === 1 ? 'PRICE ALERTS' : page === 2 ? 'EARN MONEY' : 'GET STARTED'}
          </Text>
        </View>

        {/* Title */}
        <Text style={{ fontSize:30, fontWeight:'900', color:'#FFF', textAlign:'center', lineHeight:38, marginBottom:16 }}>
          {dp.title}
        </Text>

        {/* Desc */}
        <Text style={{ fontSize:15, color:'rgba(255,255,255,0.5)', textAlign:'center', lineHeight:24, marginBottom:32 }}>
          {dp.desc}
        </Text>
              ...
              <View key={i} style={{
                ...
                <View style={{ flex:1 }}>
                  ...
                  <View style={{ flexDirection:'row', alignItems:'center', gap:8, marginTop:4 }}>
                    <Text style={{ color:'rgba(255,255,255,0.3)', fontSize:12, textDecorationLine:'line-through' }}>{item.old}</Text>
                    <Text style={{ color:'#fff', fontWeight:'900', fontSize:15 }}>{item.price}</Text>
                    <View style={{ backgroundColor:'#00C896', paddingHorizontal:8, paddingVertical:2, borderRadius:8 }}>
                      <Text style={{ color:'#fff', fontSize:10, fontWeight:'800' }}>{item.save}</Text>
                    </View>
                  </View>
                </View>
              </View>
          ...
          <View style={{ flexDirection:'row', gap:12, width:'100%' }}>
            {dp.stats.map((stat, i) => (
              <View key={i} style={{
                ...
                borderWidth:1, borderColor:'rgba(255,255,255,0.1)', padding:16, alignItems:'center',
              }}>
                <Text style={{ fontSize:22, fontWeight:'900', color: dp.color }}>{stat.value}</Text>
                <Text style={{ color:'rgba(255,255,255,0.4)', fontSize:11, marginTop:4 }}>{stat.label}</Text>
              </View>
            ))}
          </View>
        )}
      ...
      {/* Bottom */}
      <View style={{ paddingHorizontal:24, paddingBottom: Platform.OS==='ios'?48:28 }}>
        {/* Dots */}
        <View style={{ flexDirection:'row', justifyContent:'center', gap:8, marginBottom:20 }}>
          {DARK_PAGES.map((_, i) => (
            <View key={i} style={{
              ...
              width: page===i ? 28 : 8,
              backgroundColor: page===i ? dp.color : 'rgba(255,255,255,0.2)',
            }} />
          ))}
        </View>

        {/* Get Started - فقط في الصفحة الأخيرة */}
        {page === DARK_PAGES.length - 1 && (
          <TouchableOpacity
            style={{
              backgroundColor: dp.color, borderRadius:20, paddingVertical:18,
              alignItems:'center', shadowColor: dp.color, shadowOpacity:0.4, shadowRadius:15, elevation:8,
            }}
            onPress={onComplete}
            activeOpacity={0.85}
          >
        ...
        {/* تلميح السحب */}
        {page < DARK_PAGES.length - 1 && (
          <Text style={{ color:'rgba(255,255,255,0.25)', textAlign:'center', fontSize:12 }}>
            Swipe to continue →
          </Text>
        )}
      </View>
...
function AuthScreen({ onLogin }) {
  ...
  const handleSocial = async (provider) => {
    // Google OAuth requires browser redirect — not supported in APK preview
    Alert.alert(
      '🔔 Coming Soon',
          ...
          <View style={s.authTabs}>
            {[['login','Sign In'],['register','Create Account']].map(([m,l]) => (
              <TouchableOpacity key={m}
                style={mode===m ? [s.authTab, s.authTabActive] : s.authTab}
...
function DetailScreen({ product, favorites, cart, country, onBack, onFav, onCart, onBuy, onShare, onPriceAlert }) {
  ...
  const plat   = PLATFORMS[product.platform] || PLATFORMS['amazon'];
  ...
  return (
    <SafeAreaView style={{ flex:1, backgroundColor:T.bg }}>
      ...
      <View style={s.detailBottom}>
        ...
        >
          <Text style={inCart ? [s.cartCtaTxt, { color:'#fff' }] : s.cartCtaTxt}>
            {inCart ? '− Remove' : '+ Cart'}
          </Text>
        </TouchableOpacity>
...
function ShareModal({ visible, product, country, onClose }) {
  ...
  const url  = (PLATFORMS[product.platform] || PLATFORMS['amazon']).buildUrl(product, country);
  const curr = product.platform==='aliexpress' ? '$' : country.currency;
  return (
      ...
      >
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.catRow}>
          {CATEGORIES.map((c, idx) => (
            <AnimatedCatChip key={c.id} c={c} idx={idx} category={category} setCategory={setCategory} setSearch={setSearch} />
          ))}
        </ScrollView>
        ...
        <View style={s.sortRow}>
          ...
          {SORT_OPTIONS.map(so => (
            <TouchableOpacity key={so.id}
              style={sortMode===so.id ? [s.sortChip, { backgroundColor:T.primary }] : s.sortChip}
              onPress={() => setSortMode(so.id)}
            >
              <Text style={sortMode===so.id ? [s.sortChipTxt, { color:'#fff' }] : s.sortChipTxt}>{so.label}</Text>
            </TouchableOpacity>
          ))}
        </View>
        ...
        {trending.length>0 && !search && category==='all' && (
          <View style={{ marginBottom:20 }}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false}
              ...
              {trending.map((p, idx) => (
                <AnimatedTrendCard key={p.id} p={p} idx={idx} country={country} setDetail={setDetail} />
              ))}
            </ScrollView>
          </View>
        )}
        ...
        {filtered.length===0 ? (
          ...
          <View style={s.grid}>
            {filtered.map((item, idx) => (
              <AnimatedProductCard key={item.id} item={item} idx={idx} cart={cart} favorites={favorites}
